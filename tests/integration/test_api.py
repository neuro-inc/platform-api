...
class TestJobs:
    ...
    @pytest.mark.asyncio
    async def test_get_all_jobs_filter_by_name_and_statuses(
        self,
        api,
        client,
        run_job,
        ...
        # filter: name + single status
        filters = [("name", job_name), ("status", "running")]
        jobs = await jobs_client_usr1.get_all_jobs(filters)
        jobs = {job["id"] for job in jobs}
        assert jobs == {job_usr1_with_name, job_usr2_with_name}
        ...
        # filter: single status (jobs not found)
        filters = [("status", "failed")]
        jobs = await jobs_client_usr1.get_all_jobs(filters)
        jobs = {job["id"] for job in jobs}
        assert jobs == set()

        # filter: name (jobs not found) single status
    ...
    @pytest.mark.asyncio
    async def test_get_all_jobs_filter_by_name_owners_and_statuses(
        self,
        api,
        client,
        regular_user_factory,
        ...
        jobs = await jobs_client_usr1.get_all_jobs(filters)
        jobs = {job["id"] for job in jobs}
        assert jobs == {job_usr2_with_name, job_usr2_with_name_killed}
